...
public interface PortalEventStoreHelper {
	...
	IEventStore createAndConfigureEventStore(final String serializedReservationKey)
	 ...
	 * 		the serialized key for the reservation to load the event store for.
	 * @param readOnly
	 * 		{@code true} if the store should be read-only or {@code false} otherwise
	 *
	 ...
	 * 		if there isn't an event store for the provided serializedReservationKey
	 */
	IEventStore loadEventStore(final String serializedReservationKey, final boolean readOnly)
			throws InvalidParameterException;
	 ...
	 *
	 * @param serializedReservationKey
	 * 		the reservation key to check for
	 *
