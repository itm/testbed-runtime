...
public class SNAACertificate extends AbstractService implements SNAAService {
	...
	/**
	 * Access authorization for users is performed for nodes which uniform resource locator starts with these prefixes.
	 */
	protected final Provider<Set<NodeUrnPrefix>> servedNodeUrnPrefixesProvider;

	/**
	 * A security component that can access application-specific security entities such as users, roles, and permissions to
	...
	/**
	 * This cache keeps tack of all authenticated sessions for a certain amount of time.<br/> It maps a secret String which
	 * is only known to one authenticated user to the user's authentication details.<br/> This cache may be used to check
	 * whether the user was authenticated by this server recently whenever this secret String is provided.
	 */
	 ...
	 * @throws SNAAFault_Exception
	 * 		Thrown if the provided collection of node urns contains node urns with prefixes which are not served by this SNAA
	 * 		server
	 */
